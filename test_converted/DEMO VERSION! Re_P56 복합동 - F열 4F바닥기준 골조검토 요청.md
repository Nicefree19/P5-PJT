# DEMO VERSION! Re:P56 복합동 - F열 4F바닥기준 골조검토 요청 

- **From**: 이동혁 [소장] [EPC팀]
- **Date**: 2024-07-29 09:07:31
- **Source**: DEMO VERSION! Re_P56 복합동 - F열 4F바닥기준 골조검토 요청 .msg

---

안녕하세요? 프로님

요청하신 내용 반영시 일정은

이앤디몰 김대훈대표님과 상의한 결과 금주 수요일까지 작업 가능합니다.

궁금하신 내용있으면 연락주세요.

확인 부탁드립니다.

감사합니다.

--------------------------------------------------------------------------------------------------------------------------------------------------------

|  |  |
| --- | --- |
| 이동혁   EPC | 소장 | senkuzo |

|  |  |
| --- | --- |
| t: 02-2629-3104  m: 010-4249-7140  e: dhlee@senkuzo.com  www.senkuzo.com | (주)센구조연구소 SEN ENGINEERING GROUP 07226 서울시 영등포구 버드나루로 19길 6 (센스빌딩) |

> **------ Original Message ------**
>   
> **Date:** 2024-07-26 14:32:31
>   
> **From:**강상규 < sk-.kang@samsung.com >
>   
> **To:** 이동혁 [소장] [epc팀] < dhlee@senkuzo.com >,이인호 [이사] [epc팀] < ihlee@senkuzo.com >,김대훈 대표 < dhkim2630@naver.com >
>   
> **Cc:** 윤성환 < sh7752.youn@samsung.com >
>   
> **Subject:** P56 복합동 - F열 4F바닥기준 골조검토 요청
>   
>   
>
>
> 수신 : 수신자 제위
>
> 안녕하십니까 
>
> 발주처로 부터 구조검토요청이 접수되어 정리된 설계모델을 기준으로 확인요청드립니다. 
>
> ( 이엔디몰의 부재 단면 변경사항 반영등 최종모델로 검토 )
>
>   - 해석 모델 기준을 F열 P6 3개층 (4F 바닥)기준 
>
>   - 검토 사항 1~2절주 골조 변경사항 검토 (PC는 없을 것으로 예상)
>
>   - P6구간 4F에 경량지붕 마감 예정 (가벼운 하중 기준) 
>
> 일정은 가능한 29일 (차주 월요일)까지 요청드립니다. (전화 연락이 안되어 우선 일정 송부드립니다.)
>
> ---------------------------------------------------------------
>
> P5 투자비 최소화를 위해 P6 3개층 안으로 결정될 가능성이 높습니다.
>
> 이 경우 골조 외 변경사항 및 이슈사항이 있는지 확인 부탁드리고,
>
> 변경 필요 시 골조 포함 설계일정 확인 부탁드립니다. 
>
> 기존 25년 2월 공사 재개 기준으로 작성하신 일정표에 업데이트 해 주시면 될 것 같습니다.
>
> ---------------------------------------------------------
>
> 추가적으로 P6 골조 분리에 따른 F열의 설계 검토를 작성부탁드립니다.
>
> F~G열의 철골 변경은 고려대상에서 제외하겠습니다.
>
> HMB 보양등 우선 간략한 의견으로 회신 주시면 공사팀에 전달하도록 하겠습니다.
>
> 차주 월요일가지 회신부탁드립니다.
>
> 감사합니다.
>
> Best Regard.
>
>   
>
> 강상규 프로 (SANG-KYOU KANG)
>
> E&I Structural Eng’g Group
>
> SAMSUNG E&A CO., LTD.
>
> Office:  +82-2-2053-4872
>
> Mobile: +82-10-4872-5730
>
> e-mail: sk-.kang@samsung.com
>
> ![](data:image/jpeg;base64,)
>
>  **위 전자우편에 포함된 정보는 위에 기재된 수신인만을 위해 발송되는 것으로서**
>
> **보안을 유지해야 하는 정보 및 법률상****또는 다른 사유로 인하여 공개가 금지된 정보가 들어 있을 수 있습니다.**
>
> **귀하가 이 전자우편의 지정 수신인이 아니면 이를 무단으로 보유, 전송, 배포할 수 없으며,**
>
> **일부의 내용이라도 공개,****복사해서는 안됩니다.**
>
> **그러므로, 잘못 수신된 경우에는 즉시 전화 또는 전자우편 주소로 연락하여 주시고,**
>
> **원본 및 사본과 그에 따른 첨부 문서를 모두 삭제하여 주시기 바랍니다.**
>
> **Confidentiality Notice: This e-mail is intended for the addressee named in the above.**
>
> **The information contained in this e-mail is confidential and privileged. If you are not the intended recipient,**
>
> **you are hereby notified that any dissemination, distribution or copying ofthis communication is strictly prohibited.**
>
> **If you have received this e-mail in error, please notify the sender immediately and destroy this e-mail message.**
>
> **Thank you.**
>
> |  |
> | --- |
> |  |
>
> |  |
> | --- |
> |  |
>
> |  |
> | --- |
> |  |
