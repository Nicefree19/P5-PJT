# FW:FW: P5 복합동 하중 자료 송부

- **From**: 이동혁 [소장] [EPC팀]
- **Date**: 2025-11-17 06:25:35
- **Source**: FW_FW_ P5 복합동 하중 자료 송부.msg

---

| 대용량 첨부 (Big Attach ) **2개(files)** | |
| --- | --- |
| [10. 보안 서약서\_(계약전).doc](https://portal.senkuzo.com/rest/ml/mailAttachFile/bigDownload/5f770a0054a046181f39c222e098f1cc/202511170625354195093zxGPn4MKOHE "10. 보안 서약서_(계약전).doc") | (29 KB) | [Download](https://portal.senkuzo.com/rest/ml/mailAttachFile/bigDownload/5f770a0054a046181f39c222e098f1cc/202511170625354195093zxGPn4MKOHE) |
| [251113\_P5복합동 하중계획도 (건축 LAYOUT 2.1) rev6\_하중 증가량 명기(외부 발송).dwg](https://portal.senkuzo.com/rest/ml/mailAttachFile/bigDownload/5f770a0054a046181f39c222e098f1cc/202511170625354195094xV6XP2lwAWt "251113_P5복합동 하중계획도 (건축 LAYOUT 2.1) rev6_하중 증가량 명기(외부 발송).dwg") | (24 MB) | [Download](https://portal.senkuzo.com/rest/ml/mailAttachFile/bigDownload/5f770a0054a046181f39c222e098f1cc/202511170625354195094xV6XP2lwAWt) |
| 다운로드 기간(Download Term):2025-11-17~2026-05-21 | |

안녕하세요//

접수된 로딩맵 공유합니다

주연씨는 보안서약서 팀원들 (이인호,이동혁,조준호,손근영,오형석,석영진,박주연) 작성부탁해요.

영진대리는 평면 파악해서  PPT로 로딩맵 새로 업데이트 부탁합니다. 이거 하면서 주요 변경내용과 모델링할수있는 하중이 잘표현되어 있는지 분석하면서 작업해주세요.

근영차장은 내일부터 바로 모델링 작업할수있는지 타 업무와 겹치지 않도록 스케쥴 조정 부탁합니다. 어려운 부분있으면 얘기해주세요.

그리고 로딩맵을 미리 분석해주세요.

특히 P6 4층바닥하중이 적절하게 표현되어 있는지?등을 파악해보고 삼우에 추가로 요청할 사항이 없는지 파악 부탁합니다.

그외 나머지 인원들도 로딩맵 파악하고 기존과 달라진 내용 숙지하고 있으면 좋겠네요.

확인 부탁드립니다.

감사합니다.

--------------------------------------------------------------------------------------------------------------------------------------------------------

|  |  |
| --- | --- |
| 이동혁   EPC | 소장 | senkuzo |

|  |  |
| --- | --- |
| t: 02-2629-3104  m: 010-4249-7140  e: dhlee@senkuzo.com  www.senkuzo.com | (주)센구조연구소 SEN ENGINEERING GROUP 07226 서울시 영등포구 버드나루로 19길 6 (센스빌딩) |

> **------ Original Message ------**
>   
> **Date:** 2025-11-14 11:17:18
>   
> **From:**강상규 < sk-.kang@samsung.com >
>   
> **To:** 이인호 [이사] [epc팀] < ihlee@senkuzo.com >,이동혁 [소장] [epc팀] < dhlee@senkuzo.com >,김대훈 대표 < dhkim2630@naver.com >,강기림 프로 < vickysong1@naver.com >
>   
> **Cc:** 윤성환 < sh7752.youn@samsung.com >,정종근 < jk2.jung@samsung.com >
>   
> **Subject:** FW: P5 복합동 하중 자료 송부
>   
>   
>
>
> 수신 : 수신자 제위
>
> 안녕하십니까 
>
> 아래와 같이 LOADING MAP을 전달 드립니다.
>
> 변경사항 확인 후 회신 요청드립니다.
>
> 보안서약서는 작성하여 회신 바랍니다.
>
>  
>
> 감사합니다.
>
> Best Regard.
>
>   
>
> 강상규 프로 (SANG-KYOU KANG)
>
> 산업환경구조토목설계그룹
>
> SAMSUNG E&A CO., LTD.
>
> Office:  +82-2-2053-4872
>
> Mobile: +82-10-4872-5730
>
> e-mail: sk-.kang@samsung.com
>
> ![](data:image/jpeg;base64,)
>
>  위 전자우편에 포함된 정보는 위에 기재된 수신인만을 위해 발송되는 것으로서
>
> 보안을 유지해야 하는 정보 및 법률상 또는 다른 사유로 인하여 공개가 금지된 정보가 들어 있을 수 있습니다.
>
> 귀하가 이 전자우편의 지정 수신인이 아니면 이를 무단으로 보유, 전송, 배포할 수 없으며,
>
> 일부의 내용이라도 공개, 복사해서는 안됩니다.
>
> 그러므로, 잘못 수신된 경우에는 즉시 전화 또는 전자우편 주소로 연락하여 주시고,
>
> 원본 및 사본과 그에 따른 첨부 문서를 모두 삭제하여 주시기 바랍니다.
>
> Confidentiality Notice: This e-mail is intended for the addressee named in the above.
>
> The information contained in this e-mail is confidential and privileged. If you are not the intended recipient,
>
> you are hereby notified that any dissemination, distribution or copying ofthis communication is strictly prohibited.
>
> If you have received this e-mail in error, please notify the sender immediately and destroy this e-mail message. 
>
> Thank you. 
>
> --------- Original Message ---------
>
> Sender : 강동균 <ddkkang.kang@samoo.com> 그룹장/하이테크2본부/삼우종합건축사사무소
>
> Date : 2025-11-14 10:32 (GMT+9)
>
> Title : P5 복합동 하중 자료 송부
>
> To : 강상규<sk-.kang@samsung.com>
>
> CC : 윤성환<sh7752.youn@samsung.com>, 김해수<kim.6117@samoo.com>, 임미경<mk336.lim@samoo.com>, 원준석<junseok.weon@samoo.com>, samoocon04@samoo.co.kr<samoocon04@samoo.co.kr>, samoocon03@samoo.co.kr<samoocon03@samoo.co.kr>
>
>  
>
> 수신 :  E&A 강상규 프로님
>
> 삼우설계 강동균 입니다
>
> 업무에 노고가 많으십니다.
>
> P5 복합동 변경된 레이아웃을 반영한 하중평면도를 보내드립니다.
>
> 영역별 하중을 기존 하중을 참조하여 정리하였습니다.
>
> 기존 레이아웃에서 변경되어 하중이 변동된 부분을 체크하였습니다. 
>
> TBD 구간은 인접한 실의 하중을 적용하였는데, 설계 과정에서 변경될 수 있습니다.
>
> 하중에 대한 검토하시고 수정 사항이 있으면 연락주시기 바랍니다.
>
> 감사합니다.
>
> -끝-
>
> |  |
> | --- |
> |  |
>
> |  |
> | --- |
> |  |

|  |
| --- |
|  |
